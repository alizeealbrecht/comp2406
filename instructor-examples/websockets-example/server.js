...
const ROOT_DIR = 'public_html'; //dir to serve static files from
...
io.on('connection', function(socket){
    ...
    socket.on('user-logged-in', userID => {
        ...
        io.emit('receive-chat-message', 'User #' + userName + ' has logged in. Welcome them!');
        socket.emit('receive-all-messages', messages);
    });

    socket.on('chat-message-sent', message => {
        message = 'User #' + userName + ' says: ' + message;
